...
function MapDisplay3D({
  ...
  function filterDuplicates(paths: Node[]) {
    ...
    uniqueFloor.sort(
      (a, b) => (sortOrder[a] || Infinity) - (sortOrder[b] || Infinity),
    );
    return uniqueFloor;
  }
  ...
  const StrokePath: React.FC<StrokePathProps> = ({
    x1,
    y1,
    x2,
    y2,
    ...
    <line
      className={`${style} ${animationOn ? "solid-animation" : ""}`}
      x1={x1}
      y1={y1}
      x2={x2}
      y2={y2}
  ...
  const displayPath = (graph: Graph, path: string[]) => {
    ...
    if (firstNode && firstNode.floor === floor)
      pathElements.push(
        <circle
          cx={firstNode.xCoord}
          cy={firstNode.yCoord}
    ...
    if (lastNode && lastNode.floor === floor)
      pathElements.push(
        <circle
          cx={lastNode.xCoord}
          cy={lastNode.yCoord}
  ...
  const displayFloorStart = (graph: Graph, path: string[]) => {
    ...
    for (let i = 0; i < path.length - 1; i++) {
      ...
      if (
        ...
        pathElements.push(
          <circle
            id={"c" + stairsCounter + "f" + floor + "t" + nextNode.floor}
            cx={node.xCoord}
            cy={node.yCoord}
            r="50"
            fill={node.nodeType === "STAI" ? "yellow" : "blue"}
            stroke="black"
            stroke-width="10"
          />,
        );
          ...
          <g>
            {type === "start" ? (
              <img
                ...
                className="scaly-boi w-[350px] h-[350px] cancel-rotate"
              ></img>
            ) : (
              <img
                src="../../public/icon/red-pin.png"
    ...
    <div className={"relative map-rotate"}>
      <svg viewBox="0 0 5000 3400" className={"w-screen max-w-full"}>
        <image
          ...
          x="0"
          y="0"
          className="opacity-[70%] cursor-pointer dark:invert"
          onClick={() => mapChange(floor)}
        />
