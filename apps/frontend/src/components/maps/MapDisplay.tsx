...
function MapDisplay({
  ...
  const [serviceRequest, setServiceRequest] = useState<Map<string, number>>(
    new Map(),
  );
  ...
  const displayHoverInfo = (node: Node) => {
    ...
    return (
      ...
      >
        <div
          className={
            "h-fit rounded-md border bg-popover p-4 text-2xl text-popover-foreground shadow-md " +
        ...
        >
          <g>
            <img
              src={
                "../../../public/room-types/nodeType-" + node.nodeType + ".png"
              }
          ...
          <g onClick={() => clearSelection()} className="cursor-no-drop">
            {type === "start" ? (
              <img
                ...
                className="scaly-boi w-[100px] h-[100px]"
              ></img>
            ) : (
              <img
                src="../../public/icon/red-pin.png"
  ...
  const displayName = (node: Node) => {
    if (doDisplayNames && node.floor == floor && !(node.nodeType == "HALL")) {
      return (
        <text
          className="font-bold dark:invert"
          x={node.xCoord - 65}
          y={node.yCoord - 20}
          fill="black"
        >
    ...
    try {
      ...
      response.data.forEach((element: { nodeId: string }) => {
        if (requestsPerNode.has(element.nodeId)) {
          ...
          requestsPerNode.set(
            element.nodeId,
            (requestsPerNode.get(element.nodeId) as number) + 1,
          );
        } else {
    ...
    return Array.from(graph.nodes.values())
      .map((node: Node) => {
        if (node.floor == floor && doDisplayNodes) {
          const iconPath =
            iconPaths[node.nodeType] || "../../public/icon/Hall.png";
          const iconSize =
            hoverNodeId === node.id
              ? { width: 25, height: 25 }
              : { width: 20, height: 20 }; // Example sizes, adjust as needed
          if (
            !(node.nodeType == "HALL") ||
            (node.nodeType == "HALL" && doDisplayHalls)
          ) {
            return (
              <NodeStyling
                key={node.id}
                ...
                element={displayName(node)}
                nodesList={serviceRequest}
                heatmap={heatmap}
                useHeatMap={doDisplayHeatMap}
  ...
  const displayHoverCards = (graph: Graph) => {
    ...
    return Array.from(graph.nodes.values()).map((node: Node) => {
      return <g>{hoverNodeId === node.id && displayHoverInfo(node)}</g>;
    });
  };
            ...
            if (targetNode && targetNode.floor === floor) {
              edges.push(
                <line
                  ...
                  x1={node.xCoord}
                  y1={node.yCoord}
                  x2={targetNode.xCoord}
                  y2={targetNode.yCoord}
                  stroke="black"
                  strokeWidth="2"
                />,
              );
  ...
  const displayFloorChange = () => {
    ...
    return Array.from(
      pathSent.map((node: Node, index: number) => {
        if (node.floor == floor && !(floorChanges[index] == "")) {
          return (
            <g>
              <rect
                className="dark:fill-indigo-800 z-20 fill-indigo-400"
                x={node.xCoord - 64}
                y={node.yCoord - 48}
                width="125"
                height="28"
