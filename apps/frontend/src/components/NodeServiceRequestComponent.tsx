...
const TableServices: React.FC<{ tableData: ServiceRequest[] }> = ({
  tableData,
}) => {
  return (
...
export function NodeServiceRequestComponent(node: Node | null): JSX.Element {
  ...
  const [serviceArray, setServiceArray] =
    useState<ServiceRequest[]>(initialServiceArray);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response: AxiosResponse<ServiceRequest[]> = await axios.patch(
          "/api/node-populate",
          node,
          {
            headers: {
