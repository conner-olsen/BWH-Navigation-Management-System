...
import { cleaningServiceRequest } from "common/src/interfaces/interfaces.ts";
...
function GenerateTableRowsServices({
  ...
  tableData: cleaningServiceRequest[];
  employeeData: employee[];
  selectedStatus: string;
  selectedEmployeeUser: string;
  onUpdate: (data: cleaningServiceRequest[]) => void;
}): JSX.Element[] {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to fetch cleaning service requests: ${response.status}`,
          );
        }
