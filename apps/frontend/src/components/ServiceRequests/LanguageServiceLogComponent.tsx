...
function GenerateTableRowsServices({
  ...
  tableData: languageInterpreterServiceRequest[];
  employeeData: employee[];
  selectedStatus: string;
  selectedEmployeeUser: string;
  onUpdate: (data: languageInterpreterServiceRequest[]) => void;
}): JSX.Element[] {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to fetch language service requests: ${response.status}`,
          );
        }
